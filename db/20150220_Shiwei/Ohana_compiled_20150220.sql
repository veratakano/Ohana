...
BEGIN
    ...
    INSERT INTO Relation values (1, mother_id, 0, 0, father_id, NULL);
		...
		BEGIN
            ...
            IF spouse_id IS NOT NULL THEN
				IF (coor_x > spouse_x) THEN
					UPDATE coordinates set x = (x - 40) WHERE x > spouse_x and y >= coor_y or x > father_x + 20;
                ELSE
					UPDATE coordinates set x = (x - 40) WHERE x > coor_x and y >= coor_y or x > father_x + 20;
                END IF;
			ELSE
				UPDATE coordinates set x = (x - 20) WHERE x > coor_x and y >= coor_y or x > father_x + 20;
            END IF;
