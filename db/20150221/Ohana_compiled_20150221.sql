...
BEGIN
    ...
    IF (spouse_id is null) THEN
		IF member_gender = 'M' THEN
			CALL `SP_Add_Spouse`(member_id, '', '', NULL, NULL, NULL, 'F', 1,1);
		ELSE
			CALL `SP_Add_Spouse`(member_id, '', '', NULL, NULL, NULL, 'M', 1,1);
		END IF;
        ...
        IF (exists (select * from coordinates where fatherID = member_id)) THEN
			BEGIN
				...
				IF (record_count = 1) THEN
					...
					UPDATE coordinates set x = (x + 20) WHERE x = coor_x and y = (coor_y + 20) and treeID = offspring_treeID;
                ELSE
					SET SQL_SAFE_UPDATES=0;
					UPDATE coordinates set x = (x + 20) WHERE x >= coor_x and y > coor_y and treeID = offspring_treeID or x > (coor_x + 20) and treeID = offspring_treeID;
                END IF;
			END;		
		END IF;
	...
	ELSE
		BEGIN 						
			...
			IF (exists (select * from coordinates where fatherID = spouse_ID)) THEN
				...
				IF record_count = 1 THEN
					SET SQL_SAFE_UPDATES=0;
					UPDATE coordinates set x = (x + 20) WHERE x = coor_x_sp and y = (coor_y + 20) and treeID = offspring_treeID;
                ELSE
					SET SQL_SAFE_UPDATES=0;
					UPDATE coordinates set x = (x + 20) WHERE x >= coor_x_sp and y > coor_y and treeID = offspring_treeID or x > coor_x_sp + 20 and treeID = offspring_treeID;
                END IF;
			END IF;
...
BEGIN
    ...
    IF (max_x >= (father_x + 20)) THEN
		BEGIN
			IF (member_x = father_x) THEN
				SET SQL_SAFE_UPDATES=0;
				UPDATE Coordinates SET x = (x + 20) WHERE x > (father_x + 20) and treeID = member_treeID;
				UPDATE Coordinates SET x = (x + 20) WHERE x = (father_x + 20) and y >= member_y and treeID = member_treeID;
            ELSE
				SET SQL_SAFE_UPDATES=0;
				UPDATE Coordinates SET x = (x + 20) WHERE x > member_x and y >= member_y and treeID = member_treeID or x >= (member_x+20) and treeID = member_treeID;
            END IF;	
        END;
	END IF;
    ...
    ELSE
		SET SQL_SAFE_UPDATES=0;
		UPDATE Coordinates SET x = (x + 20) WHERE memberID = member_id and treeID = member_treeID;
...
-- Dump completed on 2015-02-20 16:03:40
