...
BEGIN
    ...
    IF (spouse_id is null) THEN
		IF member_gender = 'M' THEN
			CALL `SP_Add_Spouse`(member_id, '', '', NULL, NULL, NULL, 'F', 1, offspring_treeID,@spouse_id);
		ELSE
			CALL `SP_Add_Spouse`(member_id, '', '', NULL, NULL, NULL, 'M', 1, offspring_treeID,@spouse_id);
		END IF;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_Add_Parent`( 
	IN member_ID int,
    IN  father_fname varchar(35),
    IN  father_lname varchar(50),
    IN  father_dob date,
	IN  father_email varchar(50),
    IN  father_cob varchar(100),
    IN  father_vs tinyint(1),
    IN  mother_fname varchar(35),
    IN  mother_lname varchar(50),
    IN  mother_dob date,
	IN  mother_email varchar(50),
    IN  mother_cob varchar(100),
    IN  mother_vs tinyint(1),
    IN  tree_id int(11),
	OUT father_id_new int(11),
	OUT mother_id_new int(11)
 )
BEGIN
    ...
    IF (father_id = 0 and mother_id = 0) THEN
		...
		SET father_id_new = father_id;
		SET mother_id_new = mother_id;
...
BEGIN
    ...
    IF (father_id = 0 and mother_id = 0) THEN
	BEGIN
        CALL `SP_Add_Parent` (member_ID, NULL, NULL, NULL, NULL, NULL, 1, NULL, NULL, NULL, NULL, NULL, 1, sib_treeID,@father_id,@mother_id);
		SET father_id = (SELECT fatherID from Relation WHERE memberID = member_id);
		SET mother_id = (SELECT motherID from Relation WHERE memberID = member_id);
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_Add_Spouse`( 
	IN member_id int,
    IN spouse_fname varchar(35),
    IN spouse_lname varchar(50),
    IN spouse_dob date,
	IN spouse_email varchar(50),
    IN spouse_cob varchar(100),
    IN spouse_gender varchar(1),
    IN spouse_vs tinyint(1),
	IN member_treeID int(11),
	OUT spouse_id_new int(11)
 )
BEGIN
...
BEGIN
	DECLARE usr_email varchar(50) DEFAULT NULL;
    DECLARE usr_ID INT DEFAULT 0;
    DECLARE coor_x INT unsigned DEFAULT 1;
    DECLARE coor_y INT unsigned DEFAULT 1;
    DECLARE father_id, mother_id INT unsigned DEFAULT 1;
    DECLARE father_x, mother_x INT unsigned DEFAULT 1;
    DECLARE spouse_id INT unsigned DEFAULT 1;
    DECLARE spouse_x INT unsigned DEFAULT 1;
    DECLARE record_count, sib_count INT unsigned DEFAULT 1;
    ...
    SET record_count = (SELECT count(*) FROM Coordinates WHERE fatherID = member_id or motherID = member_id);
        ...
        IF (record_count > 0) THEN
			SET SQL_SAFE_UPDATES=0;
			UPDATE Member SET firstName = NULL, lastName = NULL, dateOfBirth = NULL, placeOfBirth = NULL, vitalStatus = 1, email=NULL
			WHERE memberID = member_id;
            
            DELETE FROM Rememberance WHERE MemberID = member_id and TreeID = tree_id;
			DELETE FROM Member_Image WHERE MemberID = member_id and TreeID = tree_id;
		...
		ELSE
			...
			BEGIN
				...
				SET father_x = (SELECT x FROM Coordinates WHERE memberId = father_id);
                SET mother_id = (SELECT motherID FROM Coordinates WHERE memberID = member_id);
                ...
                IF (exists(SELECT * FROM Relation WHERE memberID = member_ID)) THEN
					...
					IF spouse_id IS NOT NULL THEN
						IF (coor_x > spouse_x) THEN
							IF (sib_count = 3) THEN
								IF (spouse_x = father_x) THEN
									SET SQL_SAFE_UPDATES=0;
									UPDATE Coordinates set x = (x - 60) WHERE fatherID = father_id;
                                    UPDATE Coordinates set x = (x - 30) WHERE x > father_x + 30 and treeID = tree_id;
								ELSEIF (spouse_x = mother_x) THEN
									SET SQL_SAFE_UPDATES=0;
									UPDATE Coordinates set x = (x - 30) WHERE x > spouse_x and y >= coor_y and treeID = tree_id or x > father_x + 30 and y < coor_y and treeID = tree_id;
								END IF;
                            ELSEIF (sib_count > 3) THEN
								SET SQL_SAFE_UPDATES=0;
                        ...
                        ELSE
							...
							IF (sib_count = 3) THEN
								IF (coor_x = father_x) THEN
									SET SQL_SAFE_UPDATES=0;
									UPDATE Coordinates set x = (x - 60) WHERE fatherID = father_id;
									UPDATE Coordinates set x = (x - 30) WHERE x > father_x + 30 and treeID = tree_id;
								
								ELSEIF (coor_x = mother_x) THEN
									SET SQL_SAFE_UPDATES=0;
									UPDATE Coordinates set x = (x - 30) WHERE x > coor_x and y >= coor_y and treeID = tree_id or x > father_x + 30 and y < coor_y and treeID = tree_id;
								END IF;
                            ELSEIF (sib_count > 3) THEN
								SET SQL_SAFE_UPDATES=0;
                                UPDATE Coordinates set x = (x - 60) WHERE x > spouse_x and y >= coor_y and treeID = tree_id or x > father_x + 30 and y < coor_y and treeID = tree_id;
                        ...
                        DELETE FROM Rememberance WHERE MemberID in (member_id, spouse_id) and TreeID = tree_id;
						DELETE FROM Member_Image WHERE MemberID in (member_id, spouse_id) and TreeID = tree_id;
					ELSE
                    
						IF (sib_count > 2) THEN
							SET SQL_SAFE_UPDATES=0;
							UPDATE Coordinates set x = (x - 30) WHERE x > coor_x and y >= coor_y and treeID = tree_id or x > father_x + 30  and y < coor_y and treeID = tree_id;
						ELSEIF (coor_x = father_x) THEN
							SET SQL_SAFE_UPDATES=0;
							UPDATE Coordinates set x = (x - 30) WHERE fatherID = father_id;
                        END IF;
				...
				ELSE
                    ...
                    DELETE FROM Rememberance WHERE MemberID = spouse_id and TreeID = tree_id;
					DELETE FROM Member_Image WHERE MemberID = spouse_id and TreeID = tree_id;
...
/*!50003 SET character_set_results = @saved_cs_results */ ;
/*!50003 SET collation_connection  = @saved_col_connection */ ;
/*!50003 DROP PROCEDURE IF EXISTS `SP_DoResetPassword` */;
/*!50003 SET @saved_cs_client      = @@character_set_client */ ;
/*!50003 SET @saved_cs_results     = @@character_set_results */ ;
/*!50003 SET @saved_col_connection = @@collation_connection */ ;
/*!50003 SET character_set_client  = utf8 */ ;
...
/*!50003 SET sql_mode              = 'NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_DoResetPassword`(
	IN getemail VARCHAR(50),
	IN getPassword VARCHAR(35)
)
BEGIN
	...
	SET CheckExists = 0;
	SELECT count(*) INTO CheckExists from Users WHERE email = getemail;
	...
	IF (CheckExists > 0) THEN 
		...
		SELECT 0 AS status, firstName, lastName
		FROM Users
		WHERE email = getemail;
	ELSE
		-- don't exist
...
/*!50003 SET character_set_results = @saved_cs_results */ ;
/*!50003 SET collation_connection  = @saved_col_connection */ ;
/*!50003 DROP PROCEDURE IF EXISTS `SP_DoUpdateUser` */;
/*!50003 SET @saved_cs_client      = @@character_set_client */ ;
/*!50003 SET @saved_cs_results     = @@character_set_results */ ;
/*!50003 SET @saved_col_connection = @@collation_connection */ ;
/*!50003 SET character_set_client  = utf8 */ ;
...
/*!50003 SET sql_mode              = 'NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_DoUpdateUser`(
	IN getUserID INT(11),
	IN getfname VARCHAR(35), 
	IN getlname VARCHAR(50), 
	IN getemail VARCHAR(50),
	IN getprivacy TINYINT(1),
	IN getpwd VARCHAR(35)
)
BEGIN
...
/*!50003 SET character_set_results = @saved_cs_results */ ;
/*!50003 SET collation_connection  = @saved_col_connection */ ;
/*!50003 DROP PROCEDURE IF EXISTS `SP_DoUpdateUserFacebook` */;
/*!50003 SET @saved_cs_client      = @@character_set_client */ ;
/*!50003 SET @saved_cs_results     = @@character_set_results */ ;
/*!50003 SET @saved_col_connection = @@collation_connection */ ;
/*!50003 SET character_set_client  = utf8 */ ;
...
/*!50003 SET sql_mode              = 'NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_DoUpdateUserFacebook`(
	IN getUserID INT(11),
	IN getfbID BIGINT(20)
)
BEGIN
...
/*!50003 SET sql_mode              = 'NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_GetLogin`(
	IN getEmail varchar(50), 
	pwd VARCHAR(35), 
	fb_id BIGINT(20))
BEGIN
...
/*!50003 SET character_set_results = @saved_cs_results */ ;
/*!50003 SET collation_connection  = @saved_col_connection */ ;
/*!50003 DROP PROCEDURE IF EXISTS `SP_GetShareTree` */;
/*!50003 SET @saved_cs_client      = @@character_set_client */ ;
/*!50003 SET @saved_cs_results     = @@character_set_results */ ;
/*!50003 SET @saved_col_connection = @@collation_connection */ ;
/*!50003 SET character_set_client  = utf8 */ ;
...
/*!50003 SET sql_mode              = 'NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_GetShareTree`(
	IN getuserID INT(11)
)
BEGIN
	...
	SELECT t.treeID, u.firstName, u.lastName
	FROM Tree_Share ts 
	INNER JOIN Tree t
	ON ts.treeID = t.treeID
	INNER JOIN Users u
	ON t.userID = u.uID
	WHERE ts.uID = getuserID; 
...
/*!50003 SET character_set_results = @saved_cs_results */ ;
/*!50003 SET collation_connection  = @saved_col_connection */ ;
/*!50003 DROP PROCEDURE IF EXISTS `SP_GetTreeFirstMember` */;
/*!50003 SET @saved_cs_client      = @@character_set_client */ ;
/*!50003 SET @saved_cs_results     = @@character_set_results */ ;
/*!50003 SET @saved_col_connection = @@collation_connection */ ;
/*!50003 SET character_set_client  = utf8 */ ;
...
/*!50003 SET sql_mode              = 'NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_GetTreeFirstMember`(
	IN getTreeID INT(11)
)
BEGIN
...
/*!50003 SET character_set_results = @saved_cs_results */ ;
/*!50003 SET collation_connection  = @saved_col_connection */ ;
/*!50003 DROP PROCEDURE IF EXISTS `SP_Show_Family` */;
/*!50003 SET @saved_cs_client      = @@character_set_client */ ;
/*!50003 SET @saved_cs_results     = @@character_set_results */ ;
/*!50003 SET @saved_col_connection = @@collation_connection */ ;
/*!50003 SET character_set_client  = utf8 */ ;
...
/*!50003 SET sql_mode              = 'NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_Show_Family`( 
	member_ID int(11),
	tree_ID int(11)
)
BEGIN
...
-- Dump completed on 2015-02-27 19:34:21
